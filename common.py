...
# J.Madge For capturing and saving the input images.
...
def play_one_episode(env, func, render=False):
    # J.Madge Update the episode number.
    global num_obs, episode, time
    ...
    # TODO J.Madge 26.01.2018
    # 'ob'  : Image observation.
    # 'act' : Corresponding actions.
    ob = env.reset()
    sum_r = 0
    while True:
        act = predict(ob)
        ...
        im.save("./games/asteroids/data/" + str(num_obs) + "-" + str(episode) + "-" + str(time) + "-" + str(act) + ".png")
        print("O: " + str(num_obs) + "E: " + str(episode) + "T: " + str(time) + "A: " + str(act))

        # J.Madge next observation, next time step.
        num_obs = num_obs + 1
        time = time + 1
        if isOver:
            # J.Madge Next episode, reset time.
            time = 0
            episode = episode + 1
            return sum_r
